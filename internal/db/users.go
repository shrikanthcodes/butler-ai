...
// CheckUserExists checks if a user exists in the db for a given email, returns userID if exists
...
// CheckPassword checks if a password matches the hashed password in the db for a given userID
...
// GetUserIDByEmail retrieves a user's ID from the db for a given email
...
// UpdatePassword updates a user's password in the db
...
// UpdateEmail updates a user's email in the db
...
// UpdatePhone updates a user's phone number in the db
...
// GetUsers retrieves all users from the db
...
// DeleteAllUsers deletes all users from the db
func (DBC *DBConnector) DeleteAllUsers() error {
	return DBC.db.Delete(&entity.User{}).Error
}

// GetUsersByRole retrieves all users from the db for a given role
